...
#  2. 速度估算不再依賴繪圖 buffer（跨輪污染問题修正）
#     → 改用控制迢路內部 prev_theta 局部變數
#  3. 安全檢查每週期只呼叫一次，修正 _cycle 計數器雙重遞增 bug
#  4. round_history 賦値改在鎖內執行，消除 GUI / 控制 thread 的 race condition
#  5. _make_row 改用固定欄位名稱，避免動態 key 導致 CSV/Excel 資料靜默丟失
#  6. KILL / PAUSE / EMERGENCY 改用 threading.Event，語意明確
#  7. FORCE_EST_LIMIT 調整為合理値（QUBE 額定扇矩量級）
#  8. 魔術數字提升為具名常數
...
CURRENT_LIMIT     = 4.0                # A
FORCE_EST_LIMIT   = 1.0                # N·m  [v3] 調整為合理値

WARMUP_CYCLES       = 50
PLOT_DOWNSAMPLE_PTS = 800
POLL_INTERVAL_MS    = 200
PLOT_INTERVAL_MS    = 80
SIM_VOLT_TO_CURR    = 0.15
THREAD_WAIT_MAX     = 30
    ...
    def check(self, theta, omega, voltage, current, force_est):
        ...
        if abs(theta) > ANGLE_LIMIT_RAD:
            return self._trigger(
                f"角度超限 {math.degrees(theta):.1f}° (限制 ±{math.degrees(ANGLE_LIMIT_RAD):.0f}°)")
        if abs(omega) > SPEED_LIMIT_RADS:
            return self._trigger(
                f"速度超限 {omega:.2f} rad/s (限制 ±{SPEED_LIMIT_RADS} rad/s)")
        ...
        if not in_warmup and abs(force_est) > FORCE_EST_LIMIT:
            self._log(
                f"[WARN] 外力估算異常 {force_est:.4f} N·m (限制 {FORCE_EST_LIMIT} N·m)，本週期外力歸零")
            ...
            for rnd_idx in range(total_rounds):
                ...
                while (timeStamp < exp_time
                    ...
                    try:
                        ...
                        log_cb(f"[ERROR] 控制迢路例外：{type(e).__name__}: {e}")
                        try:
                            qube.write_voltage(0.0)
                        except Exception:
                            pass
                        safety_alert_cb(f"控制迢路例外：{type(e).__name__}: {e}")
                        break
...
class ImpedanceControlPanel(tk.Tk):
    ...
    FONT_MONO = ("Consolas", 10); FONT_BODY = ("Segoe UI", 10)
    FONT_H2   = ("Segoe UI", 11, "bold"); FONT_H1 = ("Segoe UI", 13, "bold")
    ...
    def _build_ui(self):
        ...
        tk.Label(tf, text="◈  QUBE-Servo 3", font=("Consolas", 16, "bold"),
                 bg=self.BG, fg=self.ACCENT).pack(side="left")
        tk.Label(tf, text="  阻抗控制", font=("Segoe UI", 16, "bold"),
                 bg=self.BG, fg=self.TEXT).pack(side="left")
        tk.Label(tf, text="  v3", font=("Consolas", 11),
    ...
    def _build_left(self, parent):
        ...
        for lbl, val, unit in [
            ...
            tk.Label(row, text=lbl, font=("Consolas", 9), bg=self.PANEL,
                     fg=self.SUBTEXT, width=16, anchor="w").pack(side="left")
        ...
        tk.Label(cp, text="※ 修改請直接編輯程式頂部 MOTOR PARAMETERS",
        ...
        ParamWidget(ci, "K  剛性", self._K_var, 0.0, 5.0, 0.01,
                    unit="N·m/rad", fmt="{:.3f}", slider_length=95).pack(fill="x", pady=3)
        ParamWidget(ci, "B  阻尼", self._B_var, 0.0, 1.0, 0.001,
                    unit="N·m·s/rad", fmt="{:.3f}", slider_length=95).pack(fill="x", pady=3)
        ParamWidget(ci, "M  虛擬慣量", self._M_var, 0.0, 0.5, 0.001,
                    unit="kg·m²", fmt="{:.3f}", slider_length=95).pack(fill="x", pady=3)
        self._stability_var = tk.StringVar(value="")
        tk.Label(ci, textvariable=self._stability_var, font=("Consolas", 9),
                 bg=self.PANEL, fg=self.WARN, wraplength=320,
                 justify="left").pack(anchor="w", pady=(2, 0))
        for v in [self._K_var, self._B_var, self._M_var]:
            v.trace_add("write", lambda *a: self._push_live_impedance())
        cpd = self._card(parent, "⚡  內迢路 PD 控制器")
        ...
        ParamWidget(cth, "θ_d  目標角度", self._theta_d_var,
                    -math.pi, math.pi, 0.01, unit="rad", fmt="{:.3f}",
                    slider_length=95).pack(fill="x", pady=3)
        self._theta_d_var.trace_add("write", lambda *a: self._push_live_theta_d())
        ...
        ParamWidget(ce, "實驗時長", self._exp_time_var, 5.0, 120.0, 1.0,
                    unit="s", fmt="{:.0f}", slider_length=95).pack(fill="x", pady=3)
        ParamWidget(ce, "取樣時間", self._sample_time_var, 0.001, 0.01, 0.001,
                    unit="s", fmt="{:.3f}", slider_length=95).pack(fill="x", pady=3)
        rr = tk.Frame(ce, bg=self.PANEL); rr.pack(fill="x", pady=3)
        tk.Label(rr, text="實驗次數", width=12, anchor="w",
                 font=self.FONT_BODY, bg=self.PANEL, fg=self.SUBTEXT).pack(side="left")
        tk.Spinbox(rr, textvariable=self._total_rounds_var, from_=1, to=20,
                   width=5, font=self.FONT_MONO, bg="#060a0f", fg=self.TEXT,
        ...
        tk.Label(ce, textvariable=self._round_var, font=("Consolas", 10, "bold"),
        ...
        for lbl, val in [
            ...
            tk.Label(row, text=lbl, font=("Consolas", 9), bg=self.PANEL,
                     fg=self.SUBTEXT, width=12, anchor="w").pack(side="left")
            tk.Label(row, text=val, font=("Consolas", 9, "bold"),
                     bg=self.PANEL, fg=self.WARN).pack(side="left")
        cb = self._card(parent, "")
        self._start_btn = self._btn(cb, "▶  開始實驗", self._start_control, self.ACCENT2, "#000000")
        self._pause_btn = self._btn(cb, "⏸  暫停", self._pause_control, self.WARN, "#000000")
        self._pause_btn.config(state="disabled")
        self._stop_btn = self._btn(cb, "■  停止", self._stop_control, self.DANGER, "#ffffff")
        self._stop_btn.config(state="disabled")
        self._reset_btn = self._btn(cb, "🔄  解除緊急停止", self._reset_emergency, self.ORANGE, "#000000")
        self._reset_btn.config(state="disabled")
        tk.Frame(cb, bg=self.BORDER, height=1).pack(fill="x", pady=6)
        self._btn(cb, "⬇  儲存 Excel（全部輪次）", self._download_excel_all, self.ACCENT, "#000000")
        self._btn(cb, "⬇  儲存 CSV（最後一輪）", self._download_csv_last, self.PURPLE, "#ffffff")
        self._btn(cb, "🗑  清除所有資料", self._clear_data, self.DANGER, "#ffffff")
    ...
    def _build_centre(self, parent):
        ...
        tk.Label(hdr, textvariable=self._plot_round_var,
                 font=("Consolas", 10, "bold"), bg=self.BG,
                 fg=self.ACCENT2).pack(side="left", padx=10)
        ...
        for lbl, key, clr in [
            ...
            tk.Checkbutton(tf, text=lbl, variable=v, bg=self.BG, fg=clr,
                           selectcolor=self.BG, activebackground=self.BG,
                           activeforeground=clr, font=("Consolas", 9),
                           cursor="hand2").pack(side="left", padx=2)
            self._show_vars[key] = v
        fig_bg = "#0b0f14"; ax_bg = "#0e151d"
        self._fig, axes = plt.subplots(5, 1, figsize=(7, 9.5), facecolor=fig_bg)
        self._fig.subplots_adjust(left=0.10, right=0.985, top=0.985, bottom=0.04, hspace=0.48)
        plot_cfg = [
            ("角度追蹤 (rad)",     ax_bg, "rad"),
            ("控制電壓 (V)",       ax_bg, "V"),
        ...
        ln_pos,   = axes[0].plot([], [], color=self.CLR_POS, lw=1.4, label="實際 θ")
        ln_des,   = axes[0].plot([], [], color=self.CLR_DES, lw=1.1, ls="--", alpha=0.7, label="目標 θ_d")
        ln_cmd_0, = axes[0].plot([], [], color=self.CLR_CMD, lw=1.1, ls=":",  alpha=0.8, label="修正 θ_cmd")
    ...
    def _build_right(self, parent):
        ...
        for key, lbl, unit, clr in [
            ...
            tk.Label(row, text=lbl, font=("Consolas", 9), bg=self.PANEL,
                     fg=self.SUBTEXT, width=10, anchor="w").pack(side="left")
            sv = tk.StringVar(value="—")
            tk.Label(row, textvariable=sv, font=("Consolas", 12, "bold"),
        ...
        self._alert_label = tk.Label(
            vc, text="", font=("Consolas", 9, "bold"), bg=self.PANEL,
            fg=self.DANGER, wraplength=280, justify="left")
        self._alert_label.pack(anchor="w", pady=3)
        lc = self._card(parent, "📋  系統日誌")
        lc.pack_configure(fill="both", expand=True)
        self._log_text = tk.Text(
            lc, bg="#060a0f", fg=self.SUBTEXT, font=("Consolas", 9),
            height=18, wrap="word", state="disabled", relief="flat",
        ...
        self._btn(parent, "🗑  清除日誌", self._clear_log, self.BORDER, self.TEXT)
        ...
        if control_thread and control_thread.is_alive():
            if retry >= THREAD_WAIT_MAX:
                messagebox.showwarning(
                    "警告",
                    f"舊控制 thread 無法在 {THREAD_WAIT_MAX * POLL_INTERVAL_MS // 1000} 秒內結束，請重新啟動程式。")
                self._start_btn.config(state="normal"); return
    ...
    def _download_excel_all(self):
        ...
        path = filedialog.asksaveasfilename(
            ...
            filetypes=[("Excel", "*.xlsx")], title="儲存全部輪次 Excel")
        if not path: return
        ...
        for idx, rdata in enumerate(rounds):
            ...
            for ci, h in enumerate(_ROW_FIELDS, 1):
        ...
        messagebox.showinfo("完成", f"Excel 已儲存（{len(rounds)} 個工作表）\n{path}")
    ...
    def _download_csv_last(self):
        ...
        ts_str = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        default_name = (f"impedance_K{live_K:.3f}_B{live_B:.4f}"
                        f"_M{live_M:.4f}_round{round_counter:03d}_{ts_str}.csv")
        path = filedialog.asksaveasfilename(
    ...
    def _clear_data(self):
        ...
        if not messagebox.askyesno("確認", "確定要清除所有資料？此動作無法復原。"):
            return
        with data_lock:
            all_rounds_history.clear(); round_history.clear()
            _clear_plot_buffers()
    ...
    def _card(self, parent, title):
        f = tk.Frame(parent, bg=self.PANEL, highlightbackground=self.BORDER,
    ...
    def _btn(self, parent, text, command, bg, fg):
        b = tk.Button(parent, text=text, command=command, bg=bg, fg=fg,
                      ...
                      padx=10, pady=5, activebackground=self.BORDER, cursor="hand2")
        b.pack(fill="x", pady=2)
        return b

    def _labeled_combo(self, parent, label, values, default):
        row = tk.Frame(parent, bg=self.PANEL); row.pack(fill="x", pady=2)
        tk.Label(row, text=label, width=10, anchor="w", font=self.FONT_BODY,
                 bg=self.PANEL, fg=self.SUBTEXT).pack(side="left")
